...
本專案的核心比對邏輯已抽取至 `src/compare.js`，並使用 **Node.js 內建 test runner**（零額外依賴）進行單元測試。
...
| 分類 | 說明 |
|------|------|
| `sanitize()` | 移除空白、換行、ASCII 控制字元（如 2D 條碼的 `\x1D` 群組分隔符號） |
| 完全比對（exact） | 淨化後字串須完全相同才符合 |
| 包含比對（partial） | 較短字串須包含於較長字串內才符合 |
| 邊界情況 | 空字串、Unicode 字元、未知比對規則等 |

> 測試檔案位於 `test/compare.test.js`，共 21 個測試案例。
